...
fn run_evaluation<S: ?Sized + State>(ctx: &mut LoadedContext<S>) -> AresResult<(StepState, StepState)> {
    let starting_len = ctx.stack.len();
...
where S: State
{
    ...
    match try!(run_evaluation(ctx)) {
        (StepState::Return, StepState::Complete(value)) => Ok(value),
        (next_top, top) =>
            panic!("eval(..): invalid stack state [..., {:?}, {:?}]", next_top, top),
    }
}
...
where S: State {
    ...
    match try!(run_evaluation(ctx)) {
        (StepState::Return, StepState::Complete(value)) => Ok(value),
        (next_top, top) =>
            panic!("apply(..): invalid stack state [..., {:?}, {:?}]", next_top, top),
    }
}
...
fn step_eval<S: State + ?Sized>(ctx: &mut LoadedContext<S>) -> AresResult<()> {
    ...
    if let StepState::Complete(value) = top {
        match ctx.stack.pop().unwrap() {
            ...
            StepState::EvaluatingLambda { mut bodies, env } => {
                if let Some(next_body) = bodies.pop() {
                    ctx.env_stack.push(env.clone());
        ...
        match self {
            ...
            &StepState::PreEvaluatedCallable { ref unevaluated } =>
                formatter.debug_struct("PreEvaluatedCallable")
                         ...
                         .finish(),
            &StepState::EvaluatingLambda { ref bodies, .. } =>
